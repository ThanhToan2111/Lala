...
def get_loggers(cfg):
    # Checkpoints and result CSVs are the reproducible artifacts; serializing three
    # encoder modules into Lightning's hparams YAML adds hundreds of MB per seed.
    return False


def load_for_embedding(model_class, checkpoint_path):
    device = "cuda:0" if torch.cuda.is_available() else "cpu"
    return model_class.load_from_checkpoint(checkpoint_path, map_location=device).to(device), device
...
def run_fusion_clip2(cfg, loggers):
    ...
    probe_options = {
        "fastsearch": cfg.get("probe_fastsearch", False),
        "use_sklearn": cfg.get("probe_use_sklearn", False),
    }

    def evaluate_probe(**kwargs):
        return evaluate_linear_probe(**kwargs, **probe_options)
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=os.path.join(
            cfg.results_path, "checkpoints", cfg.dataset.train_dataset, cfg.scenario, f"seed_{cfg.seed}"
        ),
    ...
    model, device = load_for_embedding(ConFu, best_model_path)
    ...
    femb12, femb13, femb23, emb1, emb2, emb3, train_labels = extract_embeddings(model, train_loader, device=device)
    femb12_t, femb13_t, femb23_t, test_emb1, test_emb2, test_emb3, test_labels = extract_embeddings(model, test_loader, device=device)
    femb12v, femb13v, femb23v, val_emb1, val_emb2, val_emb3, val_labels = extract_embeddings(model, val_loader, device=device)
    print("Extracted embeddings for train and test sets.")

    
    accuracy1 = evaluate_probe(train_feats=emb1, train_labels=train_labels, val_feats=val_emb1, val_labels=val_labels, test_feats=test_emb1, test_labels=test_labels)
    accuracy2 = evaluate_probe(train_feats=emb2, train_labels=train_labels, val_feats=val_emb2, val_labels=val_labels, test_feats=test_emb2, test_labels=test_labels)
    accuracy3 = evaluate_probe(train_feats=emb3, train_labels=train_labels, val_feats=val_emb3, val_labels=val_labels, test_feats=test_emb3, test_labels=test_labels)
    accuracy12 = evaluate_probe(train_feats=femb12, train_labels=train_labels, val_feats=femb12v, val_labels=val_labels, test_feats=femb12_t, test_labels=test_labels)
    accuracy13 = evaluate_probe(train_feats=femb13, train_labels=train_labels, val_feats=femb13v, val_labels=val_labels, test_feats=femb13_t, test_labels=test_labels)
    accuracy23 = evaluate_probe(train_feats=femb23, train_labels=train_labels, val_feats=femb23v, val_labels=val_labels, test_feats=femb23_t, test_labels=test_labels)
    accuracy12_concat = evaluate_probe(train_feats=torch.cat([emb1, emb2], dim=-1), train_labels=train_labels, val_feats=torch.cat([val_emb1, val_emb2], dim=-1), val_labels=val_labels, test_feats=torch.cat([test_emb1, test_emb2], dim=-1), test_labels=test_labels)
    accuracy13_concat = evaluate_probe(train_feats=torch.cat([emb1, emb3], dim=-1), train_labels=train_labels, val_feats=torch.cat([val_emb1, val_emb3], dim=-1), val_labels=val_labels, test_feats=torch.cat([test_emb1, test_emb3], dim=-1), test_labels=test_labels)
    accuracy23_concat = evaluate_probe(train_feats=torch.cat([emb2, emb3], dim=-1), train_labels=train_labels, val_feats=torch.cat([val_emb2, val_emb3], dim=-1), val_labels=val_labels, test_feats=torch.cat([test_emb2, test_emb3], dim=-1), test_labels=test_labels)
    ...
    accuracy_all_test = evaluate_probe(train_feats=all_train, train_labels=train_labels, val_feats=all_val, val_labels=val_labels, test_feats=all_test, test_labels=test_labels)
...
def run_symile_baseline(cfg, loggers):
    ...
    best_model_path = checkpoint_callback.best_model_path
    print(f"Best model saved at: {best_model_path}")
    model, device = load_for_embedding(SymileBaselineCLIPModule, best_model_path)
    ...
    emb1, emb2, emb3, train_labels = extract_embeddings(model, train_loader, device=device)
    val_emb1, val_emb2, val_emb3, val_labels = extract_embeddings(model, val_loader, device=device)
    test_emb1, test_emb2, test_emb3, test_labels = extract_embeddings(model, test_loader, device=device)
    print("Extracted embeddings for train and test sets.")
...
def run_triangle_baseline(cfg, loggers):
    ...
    best_model_path = checkpoint_callback.best_model_path
    print(f"Best model saved at: {best_model_path}")
    model, device = load_for_embedding(TiangleBaselineCLIPModule, best_model_path)
...
def run_gram_baseline(cfg, loggers):
    ...
    best_model_path = checkpoint_callback.best_model_path
    print(f"Best model saved at: {best_model_path}")
    model, device = load_for_embedding(GramBaselineCLIPModule, best_model_path)
...
def run_baseline_clip2(cfg, loggers):
    ...
    best_model_path = checkpoint_callback.best_model_path
    print(f"Best model saved at: {best_model_path}")
    model, device = load_for_embedding(ThreeModalityBaselineCLIPModule, best_model_path)
